# ops/launchd — the distillery heartbeat + durability (spec §7)
...
| `com.tinycloud.distillery.server` | the feed server (`bun src/server.ts` in `feed/`, after `bun run build`) | **yes** — survives reboot |
...
bash ops/launchd/install-launchd.sh
...
All three agents log to `ops/launchd/logs/` (gitignored):
