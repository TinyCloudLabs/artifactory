...
import type { ArtifactType, Audience } from "../../../skills/_shared/lib/artifact.ts";
import { ARTIFACT_TYPES } from "../../../skills/_shared/lib/artifact.ts";
...
export function buildSystemPrompt(input: GenInvocationInput): string {
  return [
    ...
    "    harness/distill-preferences/SKILL.md. NEVER touch human-authored (untagged)",
    ...
    "   surfaced ledger (index/surfaced.json) per harness/feed-run/SKILL.md —",
...
export function buildUserMessage(input: GenInvocationInput): string {
  return (
    ...
    `harness/feed-run/SKILL.md. Do not advance the deep-dive cursor.`
  );
}
