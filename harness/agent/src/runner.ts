...
function buildGenerationArgs(
  ...
  const system = [
    ...
    ...transcripts.map((t) => `    ${t}`),
    "",
    "DO, in order, from the repo root:",
    "1. Read the transcript files listed above.",
    "2. TWEET (banger-extractor → social-post): survey → pick the single most",
    "   non-obvious EARNED SECRET actually said → climb the abstraction ladder +",
    "   4-question safety test → scrub-check → verify-quotes --stamp → save.ts.",
    "   Zero is valid if no line clears the bar; aim for one. Save it UNDER",
    `   ${artifactsDir} (pass the skill's save its outDir as ${artifactsDir}).`,
    "3. ARTICLE (write-article + illustrate-card hero): draft a contract-valid",
    "   article (non-empty body, >=1 verified quote) → verify-quotes --stamp →",
    "   save.ts. Add a hero: run illustrate-card if a Gemini key is set, else set",
    "   hero_image to a real local image file in the artifact dir. Save UNDER",
    `   ${artifactsDir}.`,
    "4. CRITIC (no human gate): re-read each artifact as a skeptical editor AND a",
    "   security reviewer — non-obvious value, leak-safe, no AI-slop, every claim",
    "   anchored to a VERIFIED verbatim quote. DELETE any artifact that fails.",
    "",
    "CONSTRAINTS:",
    `- Write artifacts ONLY under ${artifactsDir} (artifacts/<type>/<slug>/).`,
    "- Do NOT publish to TinyCloud — the harness publishes survivors after you.",
    "- Anchor every claim to a verbatim transcript quote.",
    "When finished, print a one-line summary of what you saved, then stop.",
  ].join("\n");
