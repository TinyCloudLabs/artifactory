...
// per-transcript metadata + speaker turn counts, cross-transcript shared
...
import { buildDigest } from "./article.ts";

function usage(): never {
  console.error(
    "usage: bun skills/write-article/scripts/survey.ts <transcript-path>... [--max-chunk N] [--out file.json]",
  );
  process.exit(2);
}
...
if (outFile) {
  ...
  console.error(
    `Wrote ${digest.mode}-mode digest (${digest.transcriptCount} transcript(s), ${digest.chunks.length} chunks) to ${outFile}`,
  );
} else {
  console.log(json);
}
