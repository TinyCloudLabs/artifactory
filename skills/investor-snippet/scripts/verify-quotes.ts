#!/usr/bin/env bun
// verify-quotes.ts — prove every source_quote in a snippet artifact exists
// verbatim (whitespace-insensitive) in its referenced transcript's spoken
// text (parsed speaker turns, not AI-generated summary headers).
//
// Usage:
//   bun skills/investor-snippet/scripts/verify-quotes.ts <artifact.json> [--stamp]
//
// Exit 0: all quotes verified (or none present — see note). Exit 1: at least
// one failed (listed). With --stamp, full verification success writes
// quality.quotes_verified=true back into the artifact JSON (atomic write) —
// the only sanctioned way to set that flag. Never hand-set it.
//
// An investor snippet makes a CLAIM ("we closed X", "we hired Y", "a customer
// pulled us into Z"). The claim must be anchored to something actually said in
// the meeting — source_quotes are that anchor. Verification proves the words
// were spoken, not who spoke them (diarization labels can be wrong).

import { readFile, rename, writeFile } from "node:fs/promises";
import type { SourceQuote } from "../../_shared/lib/artifact.ts";
import { verifyArtifactQuotes } from "./snippet.ts";

function usage(): never {
  console.error(
    "usage: bun skills/investor-snippet/scripts/verify-quotes.ts <artifact.json> [--stamp]",
  );
  process.exit(2);
}
...
if (quotes.length === 0) {
  ...
  console.log(
    "Note: an investor snippet asserts a claim; that claim SHOULD be anchored by a verbatim source_quote. An empty list is suspicious.",
  );
  if (stampFlag) {
