#!/usr/bin/env bun
// verify-quotes.ts — prove every source_quote in a digest artifact JSON
// exists verbatim (whitespace-insensitive) in its referenced transcript.
//
// Usage:
//   bun skills/write-digest/scripts/verify-quotes.ts <artifact.json> [--stamp]
//
// Exit 0: all quotes verified. Exit 1: at least one failed (listed), or
// source_quotes is empty — digests without anchors don't ship.
...
import { verifyArtifactQuotes } from "../../_shared/lib/quotes.ts";

function usage(): never {
  console.error(
    "usage: bun skills/write-digest/scripts/verify-quotes.ts <artifact.json> [--stamp]",
  );
  process.exit(2);
}
...
if (quotes.length === 0) {
  console.error(
    "No source_quotes present. Digests must anchor every factual claim and pull quote to a transcript quote — an empty list fails verification.",
  );
  process.exit(1);
}
