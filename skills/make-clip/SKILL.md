...
a contract-valid `clip` artifact that surfaces in the Folio feed like a
podcast: an mp4 + a poster frame, stamped with the quality block.
...
The Folio feed well is `aspect-ratio: 16/10; object-fit: cover` in a 620px
column (`harness/feed/web/src/styles.css:529`); a 9:16 vertical clip gets
cropped to a sliver. **Square is feed-native + platform-portable — 1:1 is the
default.** Allow `--aspect 9:16|1:1|16:9` for explicit platform targets.

> **Feed follow-up (do NOT change feed code in this skill's PR):** wiring video
> into the feed needs a per-media aspect override (images stay 16:10, video
> 1:1) and a typed video field in the artifact contract. Flagged here; tracked
> as a follow-up.
...
The mp4 file names are recorded as `video:<file>` / `video-clean:<file>` tags
(the contract has no typed video field yet — see the feed follow-up flag),
`hero_image` points at the poster. See `skills/_shared/lib/artifact.ts` for the
full type; the `quality` block must reflect the blind-test gate + quote
verification + the `[novelty]` note.
